...
| NFR-P05 | Retention: how long `attempt_events` (≈50 rows per attempt), audit rows and disabled accounts are kept, and whether a student can be erased | not in spec | ✅ | Approved O-23; spec v0.4; privileged dry-run/batched maintenance and migration 00029. `../setup/operations.md` records the limits of structured-identity anonymization. |
...
| NFR-A05 | An outage is noticed by a person, not by a student: uptime check on `/healthz` and the SPA, with a notification | good practice | 🟡 | Fly checks affect routing, not automatic restart. Repository monitor and owner setup: `../setup/operations.md`; delivered notification still pending |
...
| NFR-F03 | The whole attempt is completable by keyboard, including the audio player and fill-blank inputs; `Esc` always works, fullscreen never traps | §10.2, §14 DoD | 🟡 | #66 fixed the token field and palette; `student-interactions.spec.ts` covers choice shortcuts, fill-blank editing and dialog isolation at three widths; complete audio/assistive-technology coverage remains **T-5.3** |
...
| NFR-U06 | The deck is authoritative: every screen is built from `docs/design/deck/`, and a departure from it is a recorded decision | `docs/design/README.md`, `docs/design/gaps.md` | 🟡 | Phase R (`70-redesign-overview.md`) rebuilds every screen to the new deck; each screen PR carries a deck comparison at five widths, light and dark |
...
| NFR-C02 | Student and public routes usable at 360 px with no horizontal scroll; safe-area insets respected | §9, §16, T-5.6 | 🟡 | Chromium checks at 360/1024/1440px; physical iOS/Android validation remains T-5.6 |
...
| NFR-C05 | Supported browsers are named (current Chrome, Safari, Firefox, Edge; iOS Safari 16+) and E2E runs on at least Chromium | §14 | 🟡 | Chromium in CI only; Safari/Firefox/Edge device validation remains pending |
...
| NFR-M02 | CI runs every check a merge needs: Contract, Web (lint, typecheck, format, unit, integration, build), E2E stubbed, E2E live against a real PG18, Server (vet, staticcheck, golangci-lint, gofmt, unit + integration + end-to-end tests), Sonar | §14 | ✅ | `.github/workflows/ci.yml`, seven jobs including Deck |
| NFR-M03 | Gitflow: features off `develop`, `--no-ff` merges, releases through `release/*` to `main`, back-merged | overview §10 | ✅ | History |
| NFR-M04 | Comments state contracts only: one paragraph on an exported identifier, none inside a function, none on unexported ones; descriptions in `openapi.yaml` are the generated docs | AGENTS.md | ✅ | Swept 2026-09-06; held in review |
| NFR-M05 | Migrations are sequential, one concern each, never edited once merged | §13.7 | ✅ | `migrations/00001`–`00029` |
| NFR-M06 | The imported deck cannot drift silently: its files are pinned and every local link resolves | `docs/design/README.md` | ✅ | CI `Deck` job runs `scripts/check-design-deck.mjs` (manifest hashes, one template and one script per page, local references) |
...
1. NFR-S13 deploy the implemented API/Pages headers and verify login/media under CSP.
...
4. NFR-A05/A06/O03 — activate the prepared availability monitor and prove owner notification delivery; error aggregation remains deferred.
...
(E04), the first authorized NFR-P05 maintenance run under approved O-23, the audit-log
screen the plan already lists under "not in v1".

**Standing.** NFR-A04 needs Neon access and confirmation of the actual restore
window. O-23 resolved the retention policy; maintenance remains an explicit
privileged operation. Each remaining gap has an issue or Phase 5 task.
