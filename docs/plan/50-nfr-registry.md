...
| NFR-U06 | The deck is authoritative: no screen is built without a board, and a board's route is what ships | `docs/design/README.md` | ✅ | Every route has a board id in its component comment; CI `Deck` runs `check.mjs` and builds the review artifact |
...
| NFR-M06 | The deck's self-check (undefined class, missing icon, unbalanced tag) runs in CI, not only by hand | `docs/design/README.md` | ✅ | CI `Deck` job runs check and artifact build on push/PR. |
