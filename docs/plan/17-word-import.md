...
- **A wake signal.** The API sends it after queueing a run (`IMPORT_WORKER_WAKE_URL`
  → the worker's `POST /wake` on `IMPORT_WORKER_WAKE_ADDR`). Signals coalesce, a
  failed send is retried briefly, and a teacher watching a queued import re-sends
  one.
...
It never polls sooner than every two seconds.
