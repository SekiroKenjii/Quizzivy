...
func choiceInput(prompt string, texts ...string) domain.Input {
	in := domain.Input{
		Type: domain.SingleChoice, Prompt: prompt, Points: "2.50", Tags: []string{},
	}
	for i, text := range texts {
		in.Options = append(in.Options, domain.OptionInput{Text: text, IsCorrect: i == 0})
	}
	return in
}
...
func TestReorderingOptionsRoundTrips(t *testing.T) {
	...
	reordered.Options = []domain.OptionInput{
...
func assertOptionOrder(t *testing.T, q domain.Question, texts []string, correct []bool) {
	t.Helper()
...
func TestBlanksAndAnswersRoundTrip(t *testing.T) {
	...
	in := domain.Input{
		Type: domain.FillBlank, Prompt: "Tôi {{1}} đi học và {{2}} về nhà",
		Points: "3.00", Tags: []string{},
		Blanks: []domain.BlankInput{
...
func TestTwoCompositionsOfOneAnswerAreStoredOnce(t *testing.T) {
	...
	in := domain.Input{
		Type: domain.FillBlank, Prompt: "Thủ đô là {{1}}",
		Points: "3.00", Tags: []string{},
