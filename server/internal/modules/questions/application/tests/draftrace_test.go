...
func TestLockForDraftUseSerialisesAgainstDelete(t *testing.T) {
	...
	for attempt := range 8 {
		...
		if !deleted && !errors.Is(deleteErr, domain.ErrReferenced) &&
			!errors.Is(deleteErr, domain.ErrNotFound) {
...
func TestLockForDraftUseRefusesADeletedQuestion(t *testing.T) {
	...
	if err := claimForDraft(ctx, pool, sectionID, q.ID); !errors.Is(err, domain.ErrNotFound) {
...
func claimForDraft(ctx context.Context, pool *pgxpool.Pool, sectionID, questionID string) error {
	...
	if err := repositories.LockForDraftUse(ctx, tx, questionID); err != nil {
		return err
	}
