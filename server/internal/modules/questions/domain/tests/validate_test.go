...
func choice(opts ...domain.OptionInput) domain.Input {
	return domain.Input{
		Type: domain.SingleChoice, Prompt: "Chọn đáp án đúng", Points: "1.00", Options: opts,
	}
}

func TestAChoiceQuestionNeedsACorrectOption(t *testing.T) {
	err := choice(
		domain.OptionInput{Text: "A", IsCorrect: false},
		domain.OptionInput{Text: "B", IsCorrect: false},
	).Validate(nil)
...
func TestSingleChoiceRejectsTwoCorrectOptions(t *testing.T) {
	err := choice(
		domain.OptionInput{Text: "A", IsCorrect: true},
		domain.OptionInput{Text: "B", IsCorrect: true},
	).Validate(nil)
	if !hasField(t, err, "options") {
...
func TestFillBlankPlaceholdersMustMatchTheBlanks(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			in := domain.Input{
				Type: domain.FillBlank, Prompt: tc.prompt, Points: "1.00",
			}
...
func TestAudioPolicyRequiresAnAudioAsset(t *testing.T) {
	policy := &domain.AudioPolicy{AllowSeek: false, ShowTranscriptAfterSubmit: true}

	t.Run("policy on an image is rejected", func(t *testing.T) {
		in := domain.Input{
			Type: domain.ShortAnswer, Prompt: "Xem ảnh", Points: "1.00",
	...
	t.Run("audio without a policy is rejected", func(t *testing.T) {
		in := domain.Input{
			Type: domain.ShortAnswer, Prompt: "Nghe", Points: "1.00", MediaAssetID: id(),
		}
	...
	t.Run("audio with a policy passes", func(t *testing.T) {
		in := domain.Input{
			Type: domain.ShortAnswer, Prompt: "Nghe", Points: "1.00",
			MediaAssetID: id(), Audio: policy,
		}
	...
	t.Run("no media and no policy passes", func(t *testing.T) {
		in := domain.Input{Type: domain.ShortAnswer, Prompt: "Viết", Points: "1.00"}
...
func TestTranscriptRequiresAudio(t *testing.T) {
	in := domain.Input{
		Type: domain.ShortAnswer, Prompt: "Viết", Points: "1.00",
...
func TestValidationReportsEveryFieldAtOnce(t *testing.T) {
	in := domain.Input{
		Type: domain.SingleChoice, Prompt: "", Points: "1.00",
