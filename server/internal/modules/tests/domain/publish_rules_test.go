...
// ValidateDraft is a pure function over a resolved DraftContent, so the rules are tested as
...
func draftWith(q domain.DraftQuestion) domain.DraftContent {
	return domain.DraftContent{
		...
		Sections: []domain.DraftSection{{
			...
			Questions: []domain.DraftQuestion{q},
		}},
	}
}

func base() domain.DraftQuestion {
...
func TestRulePointsMustBePositive(t *testing.T) {
	for _, points := range []string{"0.00", "-1.00", "not a number"} {
		...
		if v.Rule != domain.PointsPositive {
			t.Errorf("points %q gave rule %s, want %s", points, v.Rule, domain.PointsPositive)
		}
...
func TestRuleAudioPolicyNeedsAnAudioAsset(t *testing.T) {
	...
	t.Run("policy with no asset at all", func(t *testing.T) {
		...
		v := onlyViolation(t, draftWith(q))
		if v.Rule != domain.AudioQuestionHasAsset {
			t.Errorf("got rule %s, want %s", v.Rule, domain.AudioQuestionHasAsset)
		}
		if v.QuestionID != q.SourceID {
			t.Errorf("violation names %q, want %q", v.QuestionID, q.SourceID)
		}
	...
	t.Run("policy on an image asset", func(t *testing.T) {
		...
		if v := onlyViolation(t, draftWith(q)); v.Rule != domain.AudioQuestionHasAsset {
			t.Errorf("got rule %s, want %s", v.Rule, domain.AudioQuestionHasAsset)
		}
	})
...
func TestRuleChoiceNeedsACorrectOption(t *testing.T) {
	for _, questionType := range []string{"single_choice", "multiple_choice", "true_false"} {
		q := base()
		q.Type = questionType
		q.Options = []domain.DraftOption{{Ordinal: 0, Text: "A"}, {Ordinal: 1, Text: "B"}}
		if v := onlyViolation(t, draftWith(q)); v.Rule != domain.ChoiceHasCorrectOption {
...
func TestRuleBlankNeedsAnAcceptedAnswer(t *testing.T) {
	...
	q.Type = "fill_blank"
	q.Prompt = "Điền {{1}}"
	q.Blanks = []domain.DraftBlank{{Ordinal: 1}}

	if v := onlyViolation(t, draftWith(q)); v.Rule != domain.BlankHasAcceptedAnswer {
		t.Errorf("got rule %s, want %s", v.Rule, domain.BlankHasAcceptedAnswer)
	}
...
func TestRulePlaceholdersMustMatchTheBlanks(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			var invalid *domain.PublishValidationError
			if !errors.As(err, &invalid) {
				t.Fatalf("got %v, want a PublishValidationError", err)
			}
			found := false
			for _, v := range invalid.Violations {
				if v.Rule == domain.BlankPlaceholdersMatch && v.QuestionID == q.SourceID {
					found = true
				}
			}
			if !found {
				t.Errorf("no anchored %s violation in %+v", domain.BlankPlaceholdersMatch, invalid.Violations)
			}
		})
	}
}
...
func TestRuleSectionMustNotBeEmpty(t *testing.T) {
	...
	v := onlyViolation(t, d)
	if v.Rule != domain.SectionNotEmpty {
		t.Errorf("got rule %s, want %s", v.Rule, domain.SectionNotEmpty)
	}
