...
func violations(t *testing.T, b *builder, testID string) []domain.Violation {
	t.Helper()
...
func findRule(t *testing.T, got []domain.Violation, rule domain.Rule) domain.Violation {
	t.Helper()
	for _, v := range got {
		if v.Rule == rule {
			return v
...
func TestPublishRejectsAChoiceQuestionWithNoCorrectOption(t *testing.T) {
	...
	v := findRule(t, violations(t, b, draft.ID), domain.ChoiceHasCorrectOption)
	if v.QuestionID != q {
...
func TestPublishRejectsABlankWithNoAcceptedAnswer(t *testing.T) {
	...
	v := findRule(t, violations(t, b, draft.ID), domain.BlankHasAcceptedAnswer)
	if v.QuestionID != q {
		t.Errorf("violation names question %q, want %q", v.QuestionID, q)
	}
}
...
func TestPublishRejectsPlaceholdersThatDoNotMatchTheBlanks(t *testing.T) {
	...
	v := findRule(t, violations(t, b, draft.ID), domain.BlankPlaceholdersMatch)
	if v.QuestionID != q {
		t.Errorf("violation names question %q, want %q", v.QuestionID, q)
	}
}
...
func TestPublishRejectsAnEmptySection(t *testing.T) {
	...
	v := findRule(t, violations(t, b, saved.ID), domain.SectionNotEmpty)
...
func TestPublishReportsEveryProblemAtOnce(t *testing.T) {
	...
	for _, rule := range []domain.Rule{
		domain.ChoiceHasCorrectOption,
		domain.BlankHasAcceptedAnswer,
		domain.SectionNotEmpty,
	} {
		findRule(t, got, rule)
	}
	if len(got) < 3 {
...
func TestPublishingATestWithNoSectionsIsRefused(t *testing.T) {
	...
	if _, err := b.publish(created.ID); !errors.Is(err, domain.ErrNoContent) {
