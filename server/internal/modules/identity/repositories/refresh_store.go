...
func (s *Users) Rotate(ctx context.Context, tokenHash []byte, next domain.RefreshTokenRecord, now time.Time) (domain.RotateResult, error) {
	...
	case claimed.revokedAt != nil && claimed.replacedBy == nil:
		return domain.RotateResult{Outcome: domain.RotateRevoked, FamilyID: claimed.familyID}, nil
	case claimed.revokedAt != nil:
		return revokeReusedFamily(ctx, tx, claimed, next, now)
	case !claimed.expiresAt.After(now):
		return domain.RotateResult{Outcome: domain.RotateExpired, FamilyID: claimed.familyID}, nil
	}
	...
	if err != nil {
		return domain.RotateResult{}, fmt.Errorf("load token owner: %w", err)
	}
	if user.Disabled() {
...
func revokeReusedFamily(ctx context.Context, tx pgx.Tx, claimed claimedToken, next domain.RefreshTokenRecord, now time.Time) (domain.RotateResult, error) {
	...
	if err := tx.Commit(ctx); err != nil {
		return domain.RotateResult{}, fmt.Errorf("commit family revocation: %w", err)
	}
	return domain.RotateResult{Outcome: domain.RotateReused, FamilyID: claimed.familyID}, nil
}

func revokeDisabledFamily(ctx context.Context, tx pgx.Tx, claimed claimedToken, now time.Time) (domain.RotateResult, error) {
	if err := revokeFamily(ctx, tx, claimed.familyID, now); err != nil {
		return domain.RotateResult{}, err
	}
	if err := tx.Commit(ctx); err != nil {
		return domain.RotateResult{}, fmt.Errorf("commit disabled-user revocation: %w", err)
	}
