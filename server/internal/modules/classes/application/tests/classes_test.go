...
func TestAClassCarriesItsCodesMetadataAndNeverTheCode(t *testing.T) {
	...
	if got.JoinCode.MaxUses == nil || *got.JoinCode.MaxUses != domain.DefaultMaxUses {
...
func TestMembersShowHowEachOneGotIn(t *testing.T) {
	...
	bySource := map[string]domain.Member{}
...
func TestRemovingAMemberRevokesAccessAndKeepsTheirWork(t *testing.T) {
	...
	if _, err := svc.Commands.RemoveMember.Handle(ctx, command.RemoveMember{ClassID: classID, UserID: studentID, Actor: actor.Actor{ID: teacherID, IP: "203.0.113.9", UserAgent: "go-test"}}); err != nil {
...
func TestRemovingSomeoneWhoIsNotAMemberSucceedsButAMissingClassDoesNot(t *testing.T) {
	...
	if _, err := svc.Commands.RemoveMember.Handle(ctx, command.RemoveMember{ClassID: classID, UserID: studentID, Actor: actor.Actor{ID: teacherID, IP: "", UserAgent: ""}}); err != nil {
		t.Errorf("removing a non-member: %v", err)
	}
	_, err := svc.Commands.RemoveMember.Handle(ctx, command.RemoveMember{ClassID: "01935000-0000-7000-8000-00000000ffff", UserID: studentID, Actor: actor.Actor{ID: teacherID, IP: "", UserAgent: ""}})
...
func TestCreatingAClassReturnsItAndAuditsIt(t *testing.T) {
	...
	created, err := store.Create(ctx, domain.CreateInput{
...
func TestArchivingHidesAClassFromPickersAndKeepsEverything(t *testing.T) {
	...
	ctx := context.Background()
	classID, teacherID, studentID := makeClass(t, pool)
	if _, err := store.AddMember(ctx, domain.AddMemberInput{
	...
	archived, err := store.Archive(ctx, domain.ArchiveInput{
	...
	has := func(in domain.ListInput) bool {
		t.Helper()
	...
	if facets != (domain.Facets{All: 1, Joinable: 0, Archived: 1, Students: 1}) {
		t.Errorf("facets = %+v", facets)
	}

	if _, err := store.Archive(ctx, domain.ArchiveInput{
		ClassID: classID, Archived: true, ActorUserID: teacherID, Now: time.Now(),
	}); err != nil {
		t.Fatal(err)
	}
	restored, err := store.Archive(ctx, domain.ArchiveInput{
