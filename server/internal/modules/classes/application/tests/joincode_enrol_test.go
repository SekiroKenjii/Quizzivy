...
func newMember(t *testing.T) domain.NewMember {
	t.Helper()
	n := nonce(t)
...
func TestConcurrentEnrolmentsAgainstALastSeatProduceExactlyOneMember(t *testing.T) {
	...
	const racers = 6
	members := make([]domain.NewMember, racers)
	for i := range members {
	...
	for i := range racers {
		...
		go func() {
			...
			case res.Outcome == domain.PreviewOK:
				enrolled++
			case res.Outcome == domain.PreviewExhausted:
				exhausted++
			default:
...
func TestAnExpiredCodeCreatesNoUser(t *testing.T) {
	...
	if result.Outcome != domain.PreviewExpired {
...
func TestEnrollingTwiceIsIdempotentAndDoesNotBurnAUse(t *testing.T) {
	...
	for i := range 3 {
		...
		if again.Outcome != domain.PreviewOK {
...
func TestABadCodeRefusesEveryEnrolmentPathAlike(t *testing.T) {
	...
	if signup.Outcome == domain.PreviewOK {
