...
func safeFailure(f Failure) Failure {
	if !slices.Contains([]string{processingFailed, processorOutputInvalid, "SOURCE_INVALID", "SOURCE_TOO_LARGE", "SOURCE_UNSUPPORTED", "STORAGE_UNAVAILABLE", "STORAGE_QUOTA_EXCEEDED", "STORAGE_INTEGRITY_FAILED", "PROVIDER_UNAVAILABLE", "PROVIDER_RATE_LIMITED", "PROVIDER_OUTPUT_INVALID", "CONVERSION_FAILED", "CONVERSION_TIMEOUT", "CONVERSION_BUSY", "CONVERSION_CLEANUP_FAILED", "LEGACY_CONVERSION_UNAVAILABLE", "PDF_INVALID", "PDF_PROTECTED", "PDF_NO_TEXT", "PDF_TOO_LARGE", processingTimeout, interrupted, "WORKER_LEASE_LOST"}, f.Code) {
