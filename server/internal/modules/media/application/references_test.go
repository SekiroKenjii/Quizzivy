...
func TestAStudentSittingTheTestReachesItsAudio(t *testing.T) {
	...
	ok, err := repositories.ReachableByStudent(context.Background(), pool, f.sitter, f.usedAsset)
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
...
func TestAStudentWithoutAnAttemptReachesNothing(t *testing.T) {
	...
	ok, err := repositories.ReachableByStudent(context.Background(), pool, f.outsider, f.usedAsset)
	if err != nil {
		t.Fatal(err)
	}
	if ok {
...
func TestAnAssetNoVersionUsesIsNotReachable(t *testing.T) {
	...
	ok, err := repositories.ReachableByStudent(context.Background(), pool, f.sitter, f.looseAsset)
	if err != nil {
		t.Fatal(err)
	}
	if ok {
