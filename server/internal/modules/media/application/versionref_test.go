...
func publishVersionUsing(ctx context.Context, pool *pgxpool.Pool, author, assetID string, lock bool) error {
	...
	if lock {
		if err := repositories.LockForVersionUse(ctx, tx, assetID); err != nil {
			return err
		}
	}
...
func TestLockForVersionUseRefusesADeletedAsset(t *testing.T) {
	...
	if err := publishVersionUsing(ctx, pool, uploader, asset.ID, true); !errors.Is(err, domain.ErrNotFound) {
