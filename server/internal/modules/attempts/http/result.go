...
func (h Attempts) GetAttemptResult(ctx context.Context, request openapi.GetAttemptResultRequestObject) (openapi.GetAttemptResultResponseObject, error) {
	...
	for i, q := range result.Questions {
		converted, err := h.toAPIResultQuestion(ctx, principal.UserID, q)
		if err != nil {
			return nil, err
		}
...
func (h Attempts) toAPIResultQuestion(ctx context.Context, studentID string, q domain.ResultQuestion) (openapi.ResultQuestion, error) {
	base, err := h.toAPIStudentQuestion(ctx, studentID, q.Question)
	if err != nil {
	...
	if q.CorrectAnswers != nil {
		...
		for i, ba := range q.CorrectAnswers {
			...
			answers[i].BlankId = httpapi.ParseUUID(ba.BlankID)
		}
