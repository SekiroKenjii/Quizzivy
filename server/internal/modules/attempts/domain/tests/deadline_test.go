...
func TestTheDeadlineIsTheEarlierOfDurationAndClose(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			r := domain.Rules{DurationMinutes: c.duration, ClosesAt: now.Add(c.closesIn)}
...
// in the past. The CHECK (deadline_at > started_at) would reject the AttemptRecord, so
...
func TestStartingAgainstTheCloseNeverYieldsADeadlineBehindTheStart(t *testing.T) {
	now := time.Date(2026, 8, 29, 9, 0, 0, 0, time.UTC)
	r := domain.Rules{DurationMinutes: 45, ClosesAt: now.Add(2 * time.Minute)}
...
func TestTheDeadlineDoesNotMoveWhenTheAssignmentCloses(t *testing.T) {
	...
	r := domain.Rules{DurationMinutes: 60, ClosesAt: start.Add(90 * time.Minute)}
