...
func choice(correct ...bool) []domain.GradableOption {
	out := make([]domain.GradableOption, len(correct))
	for i, c := range correct {
		out[i] = domain.GradableOption{ID: string(rune('a' + i)), Ordinal: i, Correct: c}
	}
	return out
}

func TestSingleChoice(t *testing.T) {
	q := domain.GradableQuestion{Type: "single_choice", Points: 5, Options: choice(true, false, false)}
...
func TestMultipleChoiceIsAllOrNothing(t *testing.T) {
	q := domain.GradableQuestion{Type: "multiple_choice", Points: 4, Options: choice(true, true, false, false)}
...
func TestFillBlankForgivesTypingAndNotTheAnswer(t *testing.T) {
	q := domain.GradableQuestion{
		...
		Blanks: []domain.GradableBlank{{ID: "b1", Accepted: []string{"has lived", "has been living"}}},
	}
...
func TestFillBlankKeepsDiacritics(t *testing.T) {
	q := domain.GradableQuestion{
		Type: "fill_blank", Points: 3,
		Blanks: []domain.GradableBlank{{ID: "b1", Accepted: []string{"Hà Nội"}}},
	}
