...
func event(id int64, kind string, seconds float64, session string, clientSeq *int) domain.IntegrityEvent {
	return domain.IntegrityEvent{
...
func TestPairingCrossesAResumeBoundaryAndSessionsKeepTheirOwnOrder(t *testing.T) {
	events := []domain.IntegrityEvent{
		event(1, "window_blur", 10, "s1", seq(0)),
		// Skewed clock: an earlier occurred_at than the event timelineBefore it.
	...
	for i := range want {
		if kinds[i] != want[i] {
			t.Fatalf("timelineOrder %v, want %v", kinds, want)
		}
	}
	if got.Events[0].DurationMs == nil || *got.Events[0].DurationMs != 21000 {
		t.Errorf("the leave timelineBefore the resume should close on the return after it, got %v", got.Events[0].DurationMs)
	}
