...
func gradeAndClose(ctx context.Context, tx pgx.Tx, attemptID, versionID string, reason domain.Reason, deadlineAt, now time.Time) (domain.AttemptRecord, error) {
	...
	if len(ids) > 0 {
		if _, err := tx.Exec(ctx, `
			...
			return domain.AttemptRecord{}, fmt.Errorf("attempts: write auto scores: %w", err)
		}
	}
	...
	if err := tx.QueryRow(ctx,
		...
		Scan(&total); err != nil {
		return domain.AttemptRecord{}, fmt.Errorf("attempts: read total points: %w", err)
	}
	...
	if err != nil {
		return domain.AttemptRecord{}, fmt.Errorf("attempts: close attempt: %w", err)
	}
	return closed, nil
}
...
func gradingKey(ctx context.Context, tx pgx.Tx, versionID string) ([]domain.GradableQuestion, error) {
	...
	var questions []domain.GradableQuestion
	at := map[string]int{}
	for rows.Next() {
		var q domain.GradableQuestion
...
func attachKeyOptions(ctx context.Context, tx pgx.Tx, versionID string, qs []domain.GradableQuestion, at map[string]int) error {
	byQuestion, err := db.GroupBy(ctx, tx, `
		...
		func(rows pgx.Rows) (string, domain.GradableOption, error) {
			var questionID string
			var o domain.GradableOption
...
func attachKeyBlanks(ctx context.Context, tx pgx.Tx, versionID string, qs []domain.GradableQuestion, at map[string]int) error {
	byQuestion, err := db.GroupBy(ctx, tx, `
		...
		func(rows pgx.Rows) (string, domain.GradableBlank, error) {
			var questionID string
			var b domain.GradableBlank
