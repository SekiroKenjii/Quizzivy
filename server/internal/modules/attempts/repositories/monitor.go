...
func (s *Postgres) attachAttempts(ctx context.Context, assignmentID string, rows []domain.MonitorRow, at map[string]int) error {
	...
	for found.Next() {
		...
		if earned != nil && total != nil {
			row.Score = &domain.Score{Earned: *earned, Total: *total, PendingManual: pending}
		}
	}
	return found.Err()
}
...
func before(a, b domain.MonitorRow) bool {
	...
	if a.State == string(domain.InProgress) && a.DeadlineAt != nil && b.DeadlineAt != nil &&
