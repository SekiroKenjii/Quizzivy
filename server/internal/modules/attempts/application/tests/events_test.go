...
func flushOf(w world, s domain.Session, seq int, kind string) domain.FlushInput {
	return domain.FlushInput{
...
func TestAWrongOrMissingCredentialIsRefused(t *testing.T) {
	...
	events := []domain.Event{{Kind: "paste", OccurredAt: time.Now(), ClientSeq: 5}}
	cases := []struct {
		name string
		in   domain.FlushInput
	}{
		{"no credential at all", domain.FlushInput{
			AttemptID: session.Attempt.ID, SessionID: session.SessionID, Events: events}},
		{"another student's bearer", domain.FlushInput{
			AttemptID: session.Attempt.ID, SessionID: session.SessionID,
			StudentID: w.outsider, Events: events}},
		{"a beacon token that is not this attempt's", domain.FlushInput{
			AttemptID: session.Attempt.ID, SessionID: session.SessionID,
...
func away(seq int, awayMs int) domain.Event {
	return domain.Event{
...
func flushEvents(w world, s domain.Session, events ...domain.Event) domain.FlushInput {
	return domain.FlushInput{
