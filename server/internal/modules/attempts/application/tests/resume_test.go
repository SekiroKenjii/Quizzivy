...
func TestAReloadIsAResumeAndNotATakeover(t *testing.T) {
	...
	if got := eventKinds(t, pool, first.Attempt.ID); len(got) != 1 || got[0] != domain.KindResume {
...
func TestASecondDeviceOnALiveSessionIsRecordedAsATakeover(t *testing.T) {
	...
	if kinds[domain.KindSessionTakeover] != 1 {
		t.Errorf("%d session_takeover events, want 1 (kinds: %v)", kinds[domain.KindSessionTakeover], kinds)
	}
	if kinds[domain.KindResume] != 1 {
		t.Errorf("%d resume events, want 1 (kinds: %v)", kinds[domain.KindResume], kinds)
	}
}
...
func TestBackToBackReloadsDoNotInventATakeover(t *testing.T) {
	...
	for _, kind := range eventKinds(t, pool, first.Attempt.ID) {
		if kind == domain.KindSessionTakeover {
