...
func CountReferences(ctx context.Context, pool *pgxpool.Pool, assetID string) (int, error) {
	_, _, _ = ctx, pool, assetID
	return 0, nil
}
...
func ReachableByStudent(ctx context.Context, pool *pgxpool.Pool, studentID, assetID string) (bool, error) {
	_, _, _, _ = ctx, pool, studentID, assetID
	return false, nil
}
