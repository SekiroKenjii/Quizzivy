...
func TestEleventhRequestInAMinuteIsRejected(t *testing.T) {
	...
	l := ratelimit.New(100, ratelimit.PerMinute(10))
	now := time.Now()
...
func TestKeysAreIndependent(t *testing.T) {
	l := ratelimit.New(100, ratelimit.PerMinute(2))
	l.Allow("a")
	l.Allow("a")
...
func TestBothWindowsApply(t *testing.T) {
	l := ratelimit.New(100, ratelimit.PerMinute(10), ratelimit.PerHour(60))
	now := time.Now()
	l.SetClock(func() time.Time { return now })
...
func TestRejectionDoesNotDrainTheOtherBucket(t *testing.T) {
	l := ratelimit.New(100, ratelimit.PerMinute(2), ratelimit.PerHour(100))
	now := time.Now()
	l.SetClock(func() time.Time { return now })
...
func TestMemoryIsBounded(t *testing.T) {
	l := ratelimit.New(50, ratelimit.PerMinute(10))
	for i := 0; i < 500; i++ {
