...
func corsHandler() http.Handler {
	return httpx.CORS([]string{allowed})(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestWildcardIsNeverEmitted(t *testing.T) {
	h := httpx.CORS([]string{"*"})(http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {}))
	req := httptest.NewRequest(http.MethodGet, "/auth/me", nil)
